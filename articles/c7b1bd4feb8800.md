---
title: "【GCP】AppEngineにGoアプリをデプロイする【DockerとCloudSQLもあるよ】"
emoji: "🐁"
...
本記事執筆時の commit のハッシュ値は`6b03e7243f5b52ed8ebea4fa646475210bc9066e`です。
...
- この URL を見ろと言われたので見てみましょう。するとローカルから CloudSQL に接続するための認証情報を加える必要があることが分かります。
...
- こちらはスルーで OK です。サービスアカウントのアクセスをユーザーに渡すかどうかです。実験用なので使いません。今回はオーナー権限持っているアカウント(GCP にログインしている Google アカウントが自動でオーナー権限に割り当たっている)使ってしまいます。
...
- 以下コマンドを叩いてデプロイを実行します。yaml ファイルは必ずしも app.yaml である必要はないと前述しましたが、app.yaml であれば yaml ファイル名の記述を省略出来ます。名前を変えている場合はここで指定します。またプロジェクトの名前もここで指定でき、省略出来ますが複数プロジェクトで作業していて事故が起きるのが嫌なので直接指定します。5〜10 分くらいかかるので気長に待ちます。
...
- 趣味レベルで AWS も触ったことありますが、GCP の方がドキュメントがかなり読みやすかったです。
