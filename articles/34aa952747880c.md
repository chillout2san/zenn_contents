...
説明の都合上、オニオンアーキテクチャで言うインフラ層にドメインモデルを書いたり等してしまっていますが、その辺りは適宜カスタムしてご利用ください。
...
func FetchUser(ctx context.Context, userId string) (User, error) {
	...
	row, err := Db.QueryRowContext(ctx, query, userId)
...
ちなみにこんな回避方法も無くはないと思いますが、あまりスマートではなさそうです。
```go
	if err := row.Scan(&id, &name, &age); err != nil {
